...
class TestMediaConverter:
    ...
    def test_get_files_single_file(self):
        ...
        files = get_files(str(self.test_file))
        assert len(files) == 1
    ...
    def test_resolve_output_dir_custom(self):
        ...
        output_dir = resolve_output_dir(str(custom_dir))
        assert output_dir == custom_dir.resolve()
        assert output_dir.exists()
    ...
    def test_get_ffmpeg_format_valid(self):
        ...
        result = get_ffmpeg_format("mp3")
        assert result == "mp3"

    def test_get_ffmpeg_format_invalid(self):
        """Test getting invalid ffmpeg format"""
        result = get_ffmpeg_format("invalid_format")
        assert result is None

    @patch('ffmpeg.input')
    def test_convert_single_file_success(self, mock_input):
        ...
        result = convert_single_file(self.test_file, "mp3", self.temp_dir)
    ...
    @patch('ffmpeg.input')
    def test_convert_single_file_invalid_format(self, mock_input):
        """Test single file conversion with invalid format"""
        result = convert_single_file(self.test_file, "invalid_format", self.temp_dir)
        assert result is False
        mock_input.assert_not_called()

    @patch('ffmpeg.input')
    def test_convert_single_file_ffmpeg_error(self, mock_input):
    ...
    @patch('app.tools.media_converter.convert_single_file')
    @patch('app.tools.media_converter.resolve_output_dir')
    def test_convert_files(self, mock_resolve_dir, mock_convert_single):
        """Test batch file conversion"""
        mock_resolve_dir.return_value = self.temp_dir
        mock_convert_single.return_value = True
        
        files = [self.test_file]
        convert_files(files, "mp3", str(self.temp_dir))
    ...
    def test_convert_main_function_success(self, mock_convert_files, mock_get_files):
        ...
        mock_convert_files.assert_called_once_with([self.test_file], "mp3", None)

    @patch('app.tools.media_converter.get_files')
    def test_convert_main_function_error(self, mock_get_files):
    ...
    def test_print_conversion_summary(self, mock_print):
        ...
        from app.tools.media_converter import print_conversion_summary
        
        results = [True, True, False]
        print_conversion_summary(results, "mp3", self.temp_dir)
